...
function ArtistsArtwork() {
  ...
  useEffect(() => {
    const fetchArtworks = async () => {
      try {
        const response = await axios.get(`http://localhost:4000/artwork/ShowArtworksByArtistId?artistId=${id}`);
  ...
  return (
    <div className="container mx-auto px-8 py-14 bg-gray-100">
      <h1 className="text-3xl font-semibold text-center mb-8">Artworks by Artist ID</h1>
      {artworks.length === 0 ? (
